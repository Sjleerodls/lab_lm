...
from src.lab07.gpt_functions import tools, get_current_time, get_yf_stock_history, get_yf_stock_info, get_yf_stock_recommendations
...
def delta_tool_calls_to_obj(tools):
    ...
    for tool in tools:
        ...
        if tool.id is not None:     # id가 있으면
            tools_dict[tool.index]['id'] = tool.id
        if tool.function.name is not None:      # function이 있으면
            tools_dict[tool.index]['function']['name'] = tool.function.name
        if tool.type is not None:           # type이 있으면
...
def main():
    ...
    # session_state에 messages가 없으면 시스템 초기 메시지를 설정.
    ...
    if user_input:  # 사용자가 입력한 내용이 있으면
        ...
        with st.chat_message('assistant').empty():  # 비어있는 assistant 채팅 메시지를 만듦.
            for chunk in response:  # generator를 iteration함.
                ...
                if delta_content:   # 답변 조각이 있으면
                    ...
                    st.markdown(content)    # 비어있었던 assistant 채팅 창에 마크다운 형식으로 텍스트를 채움.
        ...
        # GPT가 (클라이언트에서 제공한 도구 목록 중에서) 함수 호출이 필요하다고 판단했을 때
        ...
        if tool_calls:  # GPT가 (클라이언트에서 제공한 도구 목록 중에서) 함수 호출이 필요하다고 판단했을 때
            for tool in tool_calls:
                ...
                # session_state의 messages 리스트에 'function' 역할(role)의 메시지를 추가
